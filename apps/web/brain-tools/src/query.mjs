...
export async function retrieve(question, options = {}) {
  ...
  const semanticMatches = searchIndex(index, queryEmbedding, {
    topK: Math.max((options.topK ?? config.topK) * 4, 32),
...
export async function answerQuestion(question, options = {}) {
  ...
  const intent = { relationshipQuestion, identityQuestion, threadQuestion, storyRecapQuestion, comparativeQuestion, dmAnalysisQuestion };
  const matches = curateMatchesForAnswer(retrievedMatches, question, {
  ...
  const draft = await chat([
    {
      role: "system",
      content: systemPrompt(intent, scope).join(" ")
    },
    {
      role: "user",
      content: [
  ...
  const reviewEnabled = options.review ?? config.answerReviewEnabled;
  const answer = reviewEnabled
    ? await reviewAnswer(question, draft, context, intent, scope)
    : draft;
  const displayAnswer = appendSourceAnchors(cleanAnswerForDisplay(answer), matches);

  const result = {
    answer: displayAnswer,
    sources: matches.map(sourceFromMatch)
  };
...
export async function streamAnswer(question, options = {}, callbacks = {}) {
  function emitImmediate(result) {
  ...
  if (conversationalAnswer) {
    emitImmediate(conversationalAnswer);
    return;
  }

  const operationalLogAnswer = answerOperationalLogQuestion(question);
  if (operationalLogAnswer) {
    emitImmediate(operationalLogAnswer);
    return;
  }

  const acronymAnswer = answerAcronymQuestion(question);
  if (acronymAnswer) {
    emitImmediate(acronymAnswer);
    return;
  }

  const rosterAliasIdentityAnswer = answerRosterAliasIdentityQuestion(question, options);
  if (rosterAliasIdentityAnswer) {
    emitImmediate(rosterAliasIdentityAnswer);
    return;
  }

  const missingCampaignScopeAnswer = answerMissingCampaignScopeQuestion(question, options);
  if (missingCampaignScopeAnswer) {
    emitImmediate(missingCampaignScopeAnswer);
    return;
  }

  const scopedNameGuardAnswer = answerScopedNameGuardQuestion(question, options);
  if (scopedNameGuardAnswer) {
    emitImmediate(scopedNameGuardAnswer);
    return;
  }

  const rosterPlayerAnswer = answerRosterPlayerQuestion(question, options);
  if (rosterPlayerAnswer) {
    emitImmediate(rosterPlayerAnswer);
    return;
  }

  const sourceRequestAnswer = await answerSourceRequestQuestion(question, options);
  if (sourceRequestAnswer) {
    emitImmediate(sourceRequestAnswer);
    return;
  }

  const rosterMembershipAnswer = answerRosterMembershipQuestion(question, options);
  if (rosterMembershipAnswer) {
    emitImmediate(rosterMembershipAnswer);
    return;
  }

  const rosterAnswer = await answerRosterQuestion(question, options);
  if (rosterAnswer) {
    emitImmediate(rosterAnswer);
    return;
  }

  const latestSessionAnswer = await answerLatestSessionQuestion(question, options);
  if (latestSessionAnswer) {
    emitImmediate(latestSessionAnswer);
    return;
  }

  const structuredIndexAnswer = await answerStructuredIndexQuestion(question, options);
  if (structuredIndexAnswer) {
    emitImmediate(structuredIndexAnswer);
    return;
  }

  const outOfScopeEntityAnswer = await answerOutOfScopeEntityQuestion(question, options);
  if (outOfScopeEntityAnswer) {
    emitImmediate(outOfScopeEntityAnswer);
    return;
  }

  const invalidRelationshipAnswer = await answerInvalidRelationshipQuestion(question, options);
  if (invalidRelationshipAnswer) {
    emitImmediate(invalidRelationshipAnswer);
    return;
  }

  const unknownEntityAnswer = await answerUnknownEntityQuestion(question, options);
  if (unknownEntityAnswer) {
    emitImmediate(unknownEntityAnswer);
    return;
  }

  const worldAnswer = await answerExactWorldQuestion(question, options);
  if (worldAnswer) {
    emitImmediate(worldAnswer);
    return;
  }

  const documentedRelationshipAnswer = await answerDocumentedRelationshipQuestion(question, options);
  if (documentedRelationshipAnswer) {
    emitImmediate(documentedRelationshipAnswer);
    return;
  }

  const cached = await lookupCache(question, options);
  if (cached) {
    emitImmediate(cached);
    return;
  }
  ...
  callbacks.onDone?.(finalSources, options.debug ? buildDebugTrace(question, scope, intent, retrievedMatches, matches) : null, finalAnswer);
}
...
async function reviewAnswer(question, draft, context, intent, scope) {
  return chat([
    {
      role: "system",
      content: [
...
function appendSourceAnchors(answer, matches) {
...
function sourceAnchors(matches, limit = 4) {
  const seen = new Set();
  const anchors = [];
  for (const match of matches) {
    const metadata = match.metadata ?? match;
...
function buildDebugTrace(question, scope, intent, retrievedMatches, selectedMatches) {
  return {
    question,
    scope,
    intent,
...
function systemPrompt(intent = {}, scope = resolveQueryScope("", {})) {
  const prompt = [
...
function answerAcronymQuestion(question) {
...
function answerOperationalLogQuestion(question) {
  const normalized = normalizeLoose(question);
  if (!normalized) return null;

  const asksForChangeHistory = /\b(what|which|show|list|tell|summarize|recap)\b/.test(normalized)
...
function answerConversationalPrompt(question, options = {}) {
  const normalized = normalizeLoose(question);
  if (!normalized) return null;

  if (/^(hello|hi|hey|howdy|yo|greetings|good morning|good afternoon|good evening)$/.test(normalized)) {
    return {
      answer: "Hello. Ask me something about a campaign, character, location, quest, or open thread and I will search the vault for it.",
      sources: []
    };
  }
  ...
  if (/^(thanks|thank you|thank you very much|ok|okay|cool|great|nice)$/.test(normalized)) {
    return {
      answer: "You are welcome. Send me the next campaign question when you are ready.",
      sources: []
    };
  }
  ...
  if (isLikelyOutsideGeneralKnowledgeQuestion(question)) {
    return {
      answer: "That question sits outside the campaign brain. I can answer from the vault about campaigns, characters, locations, quests, sessions, factions, artifacts, and world lore, but I should not invent a campaign answer for ordinary real-world trivia.",
      sources: []
    };
  }
  ...
  if (!looksLikeCampaignQuestion(question, options)) {
    return {
      answer: "I need a specific campaign question to search the vault. Try asking about a character, place, quest, session, faction, artifact, or open thread.",
      sources: []
    };
  }
...
function looksLikeCampaignQuestion(question, options = {}) {
...
function isQuestionLike(value) {
  return /[?]/.test(value)
    || /\b(who|what|where|when|why|how|which|tell me|summarize|recap|list|show|find|search|explain|describe|does|do|did|is|are|was|were|can|could)\b/i.test(value);
}

function hasCampaignSignal(value) {
...
function hasCampaignDomainNoun(value) {
...
function isLikelyOutsideGeneralKnowledgeQuestion(question) {
  const value = String(question ?? "").trim();
...
function resolveQueryScope(question, options = {}) {
  const selected = String(options.campaign ?? "All");
  if (isCrossCampaignWorldQuestion(question)) {
    return {
      retrievalCampaign: "All",
      promptCampaign: "World lore across campaigns",
      selectedCampaign: selected,
      crossCampaign: true,
      worldCrossCampaign: true,
  ...
  if (selected && selected !== "All") {
    ...
    if (campaign) {
      return {
        retrievalCampaign: campaign.indexName,
        promptCampaign: campaign.name,
        selectedCampaign: selected,
        crossCampaign: false,
        worldCrossCampaign: false,
        requiresCampaign: false
      };
    }
  }
  ...
  if (mentionedCampaign && !shouldAnswerAcrossCampaigns(question, options)) {
    return {
      retrievalCampaign: mentionedCampaign.indexName,
      promptCampaign: mentionedCampaign.name,
      selectedCampaign: selected,
      crossCampaign: false,
      worldCrossCampaign: false,
      requiresCampaign: false
  ...
  if (isWorldOnlyQuestion(question)) {
    return {
      retrievalCampaign: "World",
      promptCampaign: "World lore",
      selectedCampaign: selected,
      crossCampaign: false,
      worldCrossCampaign: false,
      requiresCampaign: false
  ...
  if (!shouldAnswerAcrossCampaigns(question, options)) {
    return {
      retrievalCampaign: "Unscoped",
      promptCampaign: "No campaign selected",
      selectedCampaign: selected,
      crossCampaign: false,
      worldCrossCampaign: false,
...
function answerMissingCampaignScopeQuestion(question, options = {}) {
  const scope = resolveQueryScope(question, options);
  if (!scope.requiresCampaign) return null;
  return {
    answer: "Choose a specific campaign, name the campaign in your question, or ask a world-lore question. I won't search across separate campaign histories unless the question is explicitly about the shared world or asks across campaigns.",
    sources: []
  };
}

function answerScopedNameGuardQuestion(question, options = {}) {
  ...
  return {
    ...
    sources: [
      {
        title: "The Silent Vanguard Quick Reference",
        path: "wiki/quick/The Silent Vanguard Quick Reference.md",
      ...
      },
      {
        ...
        path: "wiki/summaries/The Silent Vanguard.md",
        heading: "Campaign Scope Notes",
        campaign: "The Silent Vanguard",
        score: 1
      }
    ]
...
function answerRosterPlayerQuestion(question, options = {}) {
  const campaign = resolveQuestionCampaign(question, options);
  const targetName = extractRosterTargetName(question);
  if (!campaign && targetName) {
    const matches = Object.values(campaignsByCode)
      .filter((candidateCampaign) => campaignRosters[candidateCampaign.code])
      .map((candidateCampaign) => ({
        campaign: candidateCampaign,
        roster: campaignRosters[candidateCampaign.code],
        member: findRosterMember(campaignRosters[candidateCampaign.code], targetName)
      }))
    ...
    if (matches.length === 1) {
      ...
      return {
        answer: `${match.member.player} plays ${match.member.character} in ${match.campaign.name}.${characterDetailSentence(match.member)}`,
        sources: [{
          title: match.roster.sourceTitle,
          path: match.roster.sourcePath,
          heading: "Active Party Members",
          campaign: match.campaign.indexName,
          score: 1
        }]
      };
    }
  ...
  if (!playerMatches.length && /\blarry\b/i.test(question)) {
    ...
    if (larryMatches.length) {
      ...
      return {
        answer: `In ${campaign.name}, ${rows}.`,
        sources: [{
          title: roster.sourceTitle,
          path: roster.sourcePath,
          heading: "Active Party Members",
          campaign: campaign.indexName,
          score: 1
        }]
      };
    }
...
function answerRosterAliasIdentityQuestion(question, options = {}) {
...
function answerRosterMembershipQuestion(question, options = {}) {
  const campaign = resolveQuestionCampaign(question, options);
  if (!campaign) return null;
  ...
  return {
    answer: `No. ${candidateName} is not documented as part of ${campaign.name}'s party. ${campaign.name}'s documented party is ${formatSeries(roster.members.map((rosterMember) => rosterMember.character))}.`,
    sources: [{
      title: roster.sourceTitle,
      path: roster.sourcePath,
      heading: "Active Party Members",
...
async function answerSourceRequestQuestion(question, options = {}) {
  if (!/\b(source|sources|where\s+does|where\s+is|what\s+source)\b/i.test(question)) return null;
  if (/\blog(?:\.md)?\b/i.test(question)) {
    return {
      answer: "I do not expose the maintenance log as a player-facing source. Ask for an in-world page, character, place, faction, item, session, or thread instead.",
      sources: []
    };
  }
  ...
  if (!pages.length) {
    ...
    return {
      answer: `${entityNames[0]} is not documented in ${scopeText}, so I do not have a player-facing source page for that claim.`,
      sources: []
    };
  }

  const pageList = pages.map((page) => page.path).join(", ");
  return {
    answer: `${entityNames[0]} is documented in ${pageList}.`,
...
async function answerOutOfScopeEntityQuestion(question, options = {}) {
  const scope = resolveQueryScope(question, options);
  ...
  const inScope = exactPages.some((page) => page.campaign === scope.retrievalCampaign || page.campaign === "World" || isSharedWorldPath(page.path))
    || isRosterCharacterInCampaign(entityNames[0], scope);
  if (inScope) return null;

  const otherCampaigns = [...new Set(exactPages.map((page) => page.campaign).filter(Boolean))].join(", ");
  return {
    answer: `${entityNames[0]} is not documented in ${scope.promptCampaign}. I found that name in ${otherCampaigns}, so I am not crossing it into the selected campaign.`,
    sources: []
  };
}

async function answerUnknownEntityQuestion(question, options = {}) {
  const scope = resolveQueryScope(question, options);
  if (isRelationshipQuestion(question) || isThreadQuestion(question)) return null;
  ...
  if (!matchingPages.length) {
    ...
    const scopeText = scope.retrievalCampaign === "All" ? "the current campaign brain" : scope.promptCampaign;
    return {
      answer: `${entityName} is not documented in ${scopeText}. I should not invent a campaign answer for a name that is not in the wiki.`,
      sources: []
    };
  }

  if (scope.retrievalCampaign !== "All" && scope.retrievalCampaign !== "World") {
    const inScope = matchingPages.some((page) => page.campaign === scope.retrievalCampaign || page.campaign === "World" || isSharedWorldPath(page.path))
      || isRosterCharacterInCampaign(entityName, scope);
    if (!inScope) {
      const otherCampaigns = [...new Set(matchingPages.map((page) => page.campaign).filter(Boolean))].join(", ");
      return {
        answer: `${entityName} is not documented in ${scope.promptCampaign}. I found that name in ${otherCampaigns}, so I am not crossing it into the selected campaign.`,
        sources: []
      };
    }
  }
...
function findEntityPages(index, entityName) {
...
function isRosterCharacterInCampaign(entityName, campaign) {
  const resolvedCampaign = campaign.code ? campaign : resolveCampaignSelection(campaign.retrievalCampaign ?? campaign.indexName ?? campaign.name ?? campaign.promptCampaign);
  const roster = campaignRosters[resolvedCampaign?.code];
  if (!roster) return false;
  return Boolean(findRosterMember(roster, entityName));
}

function findRosterMember(roster, entityName) {
  const target = normalize(canonicalEntityName(entityName));
  return roster.members.find((member) => {
    const character = normalize(member.character);
    return character === target || character.startsWith(`${target} `) || target.startsWith(`${character} `);
  }) ?? null;
}

function canonicalEntityName(entityName) {
  const normalized = normalize(entityName);
  return entityAliases[normalized] ?? entityName;
}

async function answerInvalidRelationshipQuestion(question, options = {}) {
  ...
  return {
    answer: `${parts.join("; ")}. I should not invent a relationship unless both names are documented in the selected scope.`,
    sources: []
  };
}

async function answerDocumentedRelationshipQuestion(question, options = {}) {
  ...
  for (const item of index.items) {
    ...
    if (!((pageIsFirst && normalizedText.includes(second)) || (pageIsSecond && normalizedText.includes(first)))) {
      continue;
    }
  ...
  const relationshipTone = relationshipToneFromFacts(facts, firstName, secondName);
  return {
    answer: [
      relationshipTone.intro(firstName, secondName),
      facts.map((fact) => sentenceFromFact(fact)).join(" "),
      relationshipTone.close(firstName, secondName)
    ].join(" "),
    sources
  };
}

function relationshipToneFromFacts(facts, firstName, secondName) {
  ...
  const hostile = pairIncludesThreatFigure
    && /\b(enemy|hostile|threat|manipulat|possess|control|danger|fear|fiend|succubus|mother|parent|rage|vulnerability)\b/.test(text);
  if (hostile) {
    return {
      intro: (firstName, secondName) => `The vault documents ${firstName} and ${secondName} as a dangerous or unresolved relationship, not a friendly party bond.`,
      close: (firstName, secondName) => `So the safest read is that ${firstName} and ${secondName} are tied by threat, identity, and unresolved consequences; anything warmer or more trusting than that is not clearly documented.`
    };
  }

  return {
    intro: (firstName, secondName) => `The vault documents ${firstName} and ${secondName} mostly through direct party-bond notes, not as a fully explored private relationship.`,
    close: (firstName, secondName) => `So the safest read is that ${firstName} and ${secondName} have a documented practical bond in the party, with the limits and tensions above; anything deeper than that has not been written clearly yet.`
  };
}

function isRelationshipEvidenceHeading(heading) {
  return [
    "relationships and suspicions",
    "friends and allies",
    "friends and trusted bonds",
    "tensions",
    "pre campaign relationship intent",
    "pre-campaign relationship intent"
  ].includes(normalize(heading));
}

function extractRelationshipFactLines(text, firstName, secondName, pageOtherName) {
...
function sentenceFromFact(fact) {
  const trimmed = String(fact).trim();
  if (!trimmed) return "";
  return /[.!?]$/.test(trimmed) ? trimmed : `${trimmed}.`;
}

async function answerExactWorldQuestion(question, options = {}) {
  ...
  const index = await loadIndex();
  const normalizedQuestion = normalize(question);
  const worldPages = Object.values(index.pages ?? {})
    .filter((candidate) => candidate.campaign === "World");
  const aliasTitle = normalizedQuestion.includes("myrdae stories and tales")
    ? "myths and tales of myrdae"
    : null;
  const page = worldPages
    .sort((a, b) => b.title.length - a.title.length)
    .find((candidate) => normalize(candidate.title) === aliasTitle || normalizedQuestion.includes(normalize(candidate.title)));
  if (!page) return null;
  ...
  const exactWorldHeadings = routeQuestion ? [
    ...
    "summary",
    "known continents",
    "known continents regions",
    "history",
    "citizenry",
    "citizenry and enclaves",
    "traditions",
    "law",
    "laws",
    "map locations",
    ...
    "seasons",
    "major species",
    "unique myrdaen species",
    "recommended species",
    "common dialects",
    "well-known organizations",
    "recruitable adventurer-facing factions",
    "major tales",
    "mythic history",
    "prophecy",
    "bistron incident",
  ...
  const exactContext = chunks
    .map((chunk) => stripMarkdownHeadings(chunk.text))
    .join("\n\n")
    .replace(/\n{3,}/g, "\n\n")
    .trim();
  ...
  return {
    ...
    sources: [{
      title: page.title,
      path: page.path,
      heading: chunks.map((chunk) => chunk.metadata.heading).join(", "),
      campaign: "World",
      score: 1
    }]
  };
...
async function narrateExactWorldAnswer(question, context, intent = {}) {
  return chat([
    {
      role: "system",
      content: [
        ...
        intent.routeQuestion
          ? "This is a route or travel question. Name the connected places and route types in natural language, then briefly situate the place if the excerpt supports it."
          : "",
        intent.leadershipQuestion
          ? "This is a leadership or government question. Lead with who governs or enforces order if the excerpt says so, and say plainly if that detail is not provided."
          : "",
...
function narrateRouteConnections(placeName, context) {
  const routes = extractRouteConnections(context);
  if (!routes.length) {
    return `The world notes do not yet name the routes that connect to ${placeName}.`;
  }
  ...
  const routeSentences = [...byType.entries()].map(([label, locations]) => {
    return `${capitalize(label)} run between ${placeName} and ${formatSeries(locations)}.`;
  });

  const overview = extractFirstNonTableParagraph(context);
  return [
    `${placeName} is tied into Myrdae's travel web through ${formatSeries(routes.map((route) => route.connectedLocation))}.`,
    routeSentences.join(" "),
    overview ? overview : ""
  ].filter(Boolean).join(" ");
}

function extractRouteConnections(context) {
  return String(context)
    ...
    .filter(Boolean)
    .map((match) => ({
      connectedLocation: match[1].trim(),
      routeType: match[2].trim(),
      routeId: match[3].trim()
    }))
    .filter((route) => !/^connected location$/i.test(route.connectedLocation))
    .filter((route) => !/^[-:]+$/.test(route.connectedLocation))
    .filter((route) => route.connectedLocation && route.routeType && route.routeId);
}

function extractFirstNonTableParagraph(context) {
  const paragraphs = String(context)
    .split(/\n{2,}/)
...
function stripMarkdownHeadings(value) {
  return String(value)
    .split(/\r?\n/)
...
function isWorldOnlyQuestion(question) {
  ...
  return (worldSubject || pantheonSubject)
    && !/\b(in|for|from|during)\s+(HoE|SoD|Dungeons III|Dungeons 3|D3|The Silent Vanguard|Silent Vanguard|Bloody Endeavor|Wyrm Bane)\b/i.test(question);
}

function isCrossCampaignWorldQuestion(question) {
  if (
...
function isSuperlativeWorldQuestion(question) {
...
function resolveCampaignSelection(value) {
  return Object.values(campaignsByCode).find((campaign) => {
    return campaign.code === value || campaign.name === value || campaign.indexName === value;
  }) ?? null;
}

async function answerRosterQuestion(question, options = {}) {
  if (!isRosterQuestion(question)) return null;

  const campaign = resolveQuestionCampaign(question, options);
  if (!campaign && shouldAnswerAcrossCampaigns(question, options)) {
    return answerAllCampaignRosters();
  }
  if (!campaign && String(options.campaign ?? "All") === "All") {
    return {
      answer: "Choose a specific campaign, or ask for the roster across all campaigns. I won't blend campaign rosters unless you explicitly ask for that.",
      sources: []
    };
  }
  if (!campaign) return null;
  ...
  if (requestedCharacter && !roster.members.some((member) => normalize(member.character) === normalize(requestedCharacter))) {
    const index = await loadIndex();
    const exactPages = Object.values(index.pages ?? {}).filter((page) => normalize(page.title) === normalize(requestedCharacter));
    const otherCampaigns = [...new Set(exactPages.map((page) => page.campaign).filter(Boolean).filter((value) => value !== campaign.indexName))];
    ...
    return {
      answer: `${requestedCharacter} is not documented as part of ${campaign.name}'s party.${foundElsewhere} ${campaign.name}'s documented party is ${formatSeries(roster.members.map((member) => member.character))}.`,
      sources: [{
        title: roster.sourceTitle,
        path: roster.sourcePath,
        heading: "Active Party Members",
  ...
  const target = roster.members.find((member) => {
    return normalize(question).includes(normalize(member.character));
  });
  ...
  const rows = roster.members
    .map((member) => `${member.player} plays ${member.character}${characterDetailPhrase(member)}.`)
    .join(" ");

  return {
    answer: `The documented ${campaign.name} party is ${formatSeries(roster.members.map((member) => member.character))}. ${rows}`,
    sources: [{
      title: roster.sourceTitle,
      path: roster.sourcePath,
      heading: "Active Party Members",
...
function answerAllCampaignRosters() {
  const parts = Object.values(campaignsByCode)
    .filter((campaign) => campaignRosters[campaign.code])
    .map((campaign) => {
      const roster = campaignRosters[campaign.code];
      const rows = roster.members
        .map((member) => `${member.player} plays ${member.character}${characterDetailPhrase(member)}`)
        .join("; ");
      return `${campaign.name}: ${rows}.`;
    });
...
async function answerLatestSessionQuestion(question, options = {}) {
  ...
  function parseSessionNumber(page) {
    const match = (page.title || page.path).match(/Session\s+(\d+(?:\.\d+)?)/i);
...
async function answerStructuredIndexQuestion(question, options = {}) {
  const value = String(question ?? "");
  const scope = resolveQueryScope(question, options);
  const lookup = structuredIndexLookup(value);
  if (!lookup) return null;

  const pagePath = path.join(config.vaultRoot, ...lookup.path.split("/"));
  let raw = "";
  try {
  ...
  return {
    ...
    sources: [{
      title: lookup.sourceTitle,
      path: lookup.path,
      heading: lookup.heading,
      campaign: scope.retrievalCampaign === "World" ? "World" : "All",
      score: 1,
      semanticScore: 0,
      lexicalScore: 1,
      directScore: 1
...
function structuredIndexLookup(question) {
  ...
  if (/\b(open|active|unresolved|current)\b/i.test(question) && /\b(thread|threads|quest|quests|lead|leads|plot|plots)\b/i.test(question)) {
    return {
      title: "Open threads",
      sourceTitle: "Open Threads By Campaign",
  ...
  if (/\b(faction|factions|group|groups|guild|guilds|organization|organizations)\b/i.test(question)) {
    return {
      title: "Faction index",
      sourceTitle: "Factions And Groups Index",
      path: "wiki/indexes/Factions And Groups Index.md",
      heading: "Factions",
      limit: 16
    };
  }
  if (/\b(location|locations|place|places|city|cities|town|towns|where)\b/i.test(question) && /\b(list|index|all|known|major|important|active|current)\b/i.test(question)) {
    return {
      ...
      sourceTitle: "Locations Index",
      path: "wiki/indexes/Locations Index.md",
      heading: "Locations",
      limit: 16
    };
  }
  if (!namedEntities.length && /\b(route|routes|road|roads|connect|connections|travel|path|paths)\b/i.test(question) && /\b(list|index|all|known|major|overview)\b/i.test(question)) {
    return {
      ...
      sourceTitle: "World Map Location Index",
      path: "wiki/world/World Map Location Index.md",
  ...
  if (/\b(god|gods|deity|deities|pantheon|divine)\b/i.test(question)) {
    return {
      ...
      sourceTitle: "Pantheon of Myrdae",
      path: "wiki/concepts/Pantheon of Myrdae.md",
      heading: "Pantheon",
      limit: 16
    };
  }
...
function selectStructuredLines(markdown, scope, question, lookup) {
  const campaign = scope.retrievalCampaign;
  ...
  for (const rawLine of markdown.split(/\r?\n/)) {
    ...
    if (entityNames.length && !entityNames.some((name) => normalizedText.includes(name))) {
      if (routeQuestion || lookup.path.includes("World Map Location Index")) {
        continue;
      }
    }
    if (campaign !== "All" && currentCampaignSection === "All" && normalizedCampaign && !normalizedText.includes(normalizedCampaign) && !lookup.path.includes("Pantheon")) {
      const mentionsAnyCampaign = Object.values(campaignsByCode).some((candidate) => normalizedText.includes(normalize(candidate.indexName)) || normalizedText.includes(normalize(candidate.name)));
...
function shouldAnswerAcrossCampaigns(question, options = {}) {
...
function isRosterQuestion(question) {
...
function extractRosterTargetName(question) {
...
function characterDetailSentence(member) {
...
function characterDetailPhrase(member) {
  const speciesKnown = member.species && member.species !== "Unknown";
  const classKnown = member.className && member.className !== "Unknown";
...
function resolveQuestionCampaign(question, options = {}) {
  if (options.campaign && options.campaign !== "All") {
    return Object.values(campaignsByCode).find((campaign) => campaign.code === options.campaign || campaign.name === options.campaign) ?? null;
  }

  const normalized = normalize(question);
  return Object.values(campaignsByCode).find((campaign) => {
    return normalized.includes(normalize(campaign.name))
      || normalized.includes(normalize(campaign.code))
      || campaign.aliases.some((alias) => normalized.includes(normalize(alias)));
  }) ?? null;
}
...
const campaignsByCode = {
  HoE: {
    ...
    indexName: "HoE",
    aliases: ["heroes of emberstran", "heart of emberstran", "hoe"]
  },
  SoD: {
    code: "SoD",
    name: "Souls of Destiny",
    indexName: "SoD",
    aliases: ["souls of destiny", "sod"]
  },
  TSV: {
    code: "TSV",
    name: "The Silent Vanguard",
    indexName: "The Silent Vanguard",
    aliases: ["the silent vanguard", "silent vanguard", "tsv"]
  },
  WB: {
    code: "WB",
    name: "Bloody Endeavor",
    indexName: "Bloody Endeavor",
    aliases: ["bloody endeavor", "wyrm bane", "wb"]
  },
  D3: {
    code: "D3",
    name: "Dungeons III",
    indexName: "Dungeons III",
    aliases: ["dungeons iii", "dungeons 3", "d3"]
  },
  TCB: {
    code: "TCB",
    name: "The Crystal Bottle",
    indexName: "The Crystal Bottle",
    aliases: ["the crystal bottle", "crystal bottle", "tcb"]
  }
};
...
function curateMatchesForAnswer(matches, question, intent) {
  const recapLikeQuestion = intent.threadQuestion || intent.storyRecapQuestion;
  const maxMatches = intent.relationshipQuestion ? 12 : intent.identityQuestion ? 18 : recapLikeQuestion ? 20 : isNarrowQuestion(question) ? 10 : 16;
  const perPageLimit = intent.relationshipQuestion ? 3 : intent.identityQuestion ? 5 : recapLikeQuestion ? 5 : 3;
  const selected = [];
...
function isAnswerMaintenanceHeading(heading) {
  return [
    "scope",
    "campaign scope",
    "source type",
...
function isNarrowQuestion(question) {
  ...
  return matches
    .map((match) => {
      ...
      return {
        ...match,
        directScore,
        score: match.score * 0.84 + directScore * 0.16
      };
    })
    .sort((a, b) => b.score - a.score);
}

function directAnswerScore(question, match) {
  ...
  const headingHits = tokens.filter((token) => heading.includes(token)).length;
...
function importantQuestionTokens(question) {
  const stopwords = new Set([
    "a", "an", "and", "are", "about", "does", "for", "from", "in", "is", "it", "of",
    "the", "to", "was", "what", "who", "where", "when", "why", "how", "with", "all",
    "tell", "me", "know", "do", "you", "this", "that"
  ]);
...
function expandCoMentionMatches(index, matches, question, options) {
  ...
  const matchesById = new Map(expanded.map((match) => [match.id, match]));
  const coMentions = index.items
    ...
    .map((item) => ({
      ...item,
      score: coMentionScore(item)
    }))
    .sort((a, b) => b.score - a.score);

  for (const candidate of coMentions) {
    addOrBoostMatch(expanded, matchesById, candidate);
    if (expanded.length >= Math.max(config.topK + 22, matches.length + 8)) break;
  }

  return expanded;
}

function expandEntityMatches(index, semanticMatches, question, options) {
  const entityNames = extractEntityNames(question);
  if (!entityNames.length) return semanticMatches;

  const expanded = [...semanticMatches];
  const matchesById = new Map(expanded.map((match) => [match.id, match]));
  const minimumExpandedResults = Math.max(config.topK + 17, semanticMatches.length + 17);
  ...
  for (const entityName of entityNames) {
    ...
    const candidates = index.items
      ...
      .map((item) => ({
        ...item,
        score: entityExpansionScore(item, entityName)
      }))
      .sort((a, b) => b.score - a.score);

    for (const candidate of candidates) {
      const page = candidate.metadata.path;
      if (!candidatesByPage.has(page)) candidatesByPage.set(page, []);
      candidatesByPage.get(page).push(candidate);
    }
  ...
  return expanded
    .sort((a, b) => b.score - a.score)
    .slice(0, Math.max(minimumExpandedResults, semanticMatches.length));
}

function expandGraphMatches(index, matches, question, options) {
  ...
  const graphCandidates = index.items
    ...
    .map((item) => ({
      ...item,
      score: graphExpansionScore(item, anchorPages)
    }))
    .sort((a, b) => b.score - a.score);
  ...
  for (const candidate of graphCandidates) {
    ...
    if (expanded.length >= Math.max(config.topK + 24, matches.length + 8)) break;
  }

  return expanded
    .sort((a, b) => b.score - a.score)
    .slice(0, Math.max(config.topK + 24, matches.length));
}

function addOrBoostMatch(matches, matchesById, candidate) {
...
function selectAnchorPages(index, matches, question, options) {
...
function extractEntityNames(question) {
  ...
  return [...question.matchAll(/\b[A-Z][A-Za-z']{2,}(?:\s+[A-Z][A-Za-z']{2,}){0,3}\b/g)]
    .map((match) => cleanEntityName(match[0]))
    .filter((name) => !["Who", "What", "Where", "When", "Why", "How"].includes(name))
    .filter((name) => !isCampaignReferenceName(name));
}

function isRelationshipQuestion(question) {
  return hasRelationshipKeyword(question)
    && extractEntityNames(question).length >= 2;
}
...
function hasRelationshipKeyword(question) {
...
function isThreadQuestion(question) {
  return /\b(thread|plot|quest|lead|mystery|storyline|story line|arc)\b/i.test(question);
}

function isStoryRecapQuestion(question) {
  const value = String(question ?? "");
  return /\b(summarize|summary|recap|what happened|session recap|tell me what happened|catch me up|previously on)\b/i.test(value)
    || (/\b(session|episode|last night|last session)\b/i.test(value) && /\b(what|summarize|recap|tell|happened)\b/i.test(value));
}
...
function isCampaignReferenceName(value) {
  const normalized = normalize(value);
  return Object.values(campaignsByCode).some((campaign) => {
    return normalized === normalize(campaign.code)
      || normalized === normalize(campaign.name)
      || campaign.aliases.some((alias) => normalized === normalize(alias));
  });
}
...
function pageAllowed(page, options) {
...
function isEntityRelated(item, entityName) {
...
function entityExpansionScore(item, entityName) {
  const normalizedName = normalize(entityName);
  const title = normalize(item.metadata.title);
  ...
  const headingPriority = [
    ...
    "religion and values",
    "motivations",
    "enemies threats and suspicions",
    "strong inferences",
    "directly documented sequence"
  ];
...
function graphExpansionScore(item, anchorPages) {
  const heading = normalize(item.metadata.heading);
  let score = 0.72;
...
function coMentionScore(item) {
  ...
  if ([
    ...
    "enemies threats and suspicions",
    "goals and active leads",
    "confirmed personal thread",
    "open gaps",
    "current leads",
    "known bonds",
    "tensions"
  ...
  if (/\b(theft|lies|vision|suspect|challenge|accountable|detect thoughts|hidden truths)\b/i.test(item.text)) {
    score += 0.08;
  }
  return score;
}

function pagePriority(pagePath, entityName) {
