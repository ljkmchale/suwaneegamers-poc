...
export async function InsightsPage({ view = "overview", searchParams }: { view?: InsightView; searchParams?: Promise<InsightParams> }) {
  ...
  return <div className="mx-auto max-w-[90rem] space-y-6">
    ...
    {view === "maps" && map && <>
      ...
      {table("Map visit journeys",[c("previous","Previous page"),c("next","Next page"),n("visits","Visits"),n("opens","Map opens")],map.journeys,"Page movement within each visit and selected period. An absent next page is not proof of abandonment.")}
      {table("Map visits",[c("visitor","Visitor",undefined,"href"),c("cohort","Retained-history cohort"),c("source","Acquisition"),c("campaign","Campaign"),c("entry","Acquisition / entry page"),c("device","Device"),n("views","Map views"),n("clicks","Selections"),sec("engagedSeconds","Engaged"),dt("lastSeen","Last map activity")],map.visits.map((r)=>({...r,href:href("maps",{visit:r.id})})),"Select a visit to follow its recorded sequence. Use Exclude internal to focus on activity outside known owner/testing traffic.")}
      {params.visit && <div className="space-y-3"><div className="flex justify-between gap-3"><h2 className="font-cinzel">{map.selectedVisit?.visitor ?? "Visit not in this selection"}</h2><Link href={href("maps")} className="text-sm text-violet-300">Clear visit selection</Link></div>{table("Visit timeline",[dt("at","Time"),c("path","Page"),c("event","Event"),c("kind","Type"),c("target","Target"),n("value","Recorded value")],map.timeline,"Chronological events inside the period. Values are seconds for engagement, percent for depth, and milliseconds for loads.")}</div>}
    </>}
    {view === "operations" && <>
