  ...
  return <div className="mx-auto max-w-[90rem] space-y-6">
    ...
    <div className={`${panel} space-y-4`}>
      <form className="flex flex-wrap items-end gap-4" action={href(view).split("?")[0]}>
        ...
        <label className="text-xs text-[#baacc8]">Audience<select name="audience" defaultValue={audience} className="mt-1 block max-w-full rounded border border-[#493957] bg-[#08050f] px-3 py-2 text-sm text-[#e8dfc8]">{Object.entries(AUDIENCE_LABELS).filter(([key]) => key !== "all").map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>
