// The Souls of Destiny Chronicle builder — turns sod-chronicle.md into a self-contained HTML page.
// Usage:  node build-chronicle.mjs            (reads/writes this folder)
//         node build-chronicle.mjs <dir> <out.html>
// Inputs (same folder): sod-chronicle.md, chronicle.css, session-images.json
import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
const HERE = path.dirname(fileURLToPath(import.meta.url));
const DIR = process.argv[2] || HERE;
const OUT = process.argv[3] || path.join(DIR, 'souls-of-destiny-chronicle.html');
...
function parseSession(lines){
  const head=lines[0].match(/^(\d+)\s*[–-]\s*(.+?)\s*$/);
...
<section class="session" id="s${s.num}" data-theme-art="${theme}" style="--gx:${gx}%;--gy:${gy}%">
  <header class="hero">
    ...
    <div class="hero-inner">
      <div class="kicker">The Souls of Destiny · Chapter ${parseInt(s.num,10)}</div>
...
const md=fs.readFileSync(DIR+'/sod-chronicle.md','utf8');
...
const html=`<title>The Souls of Destiny Chronicle</title>
...
<div class="chronicle">
  <aside class="rail">
    <a class="rail-back" href="/campaigns/souls-of-destiny" aria-label="Back to Souls of Destiny campaign">&larr; <span>Souls of Destiny</span></a>
    <div class="rail-head">
      <div class="rail-title">Souls of Destiny<br>Chronicle</div>
      <div class="rail-sub">A living account, as the party tells it</div>
    </div>
    <nav><ol class="rail-list">${rail}</ol></nav>
    <div class="rail-foot">${sessions.length} chapters &middot; Souls of Destiny</div>
  </aside>

  <main class="scroll">
    ${sessions.map(renderSession).join('\n')}
    <footer class="pagefoot">Suwanee Gamers &middot; Souls of Destiny Chronicle</footer>
  </main>
</div>
