// The A New Adventure Chronicle builder — turns a-new-adventure-chronicle.md into a self-contained HTML page.
// Usage:  node build-chronicle.mjs            (reads/writes this folder)
//         node build-chronicle.mjs <dir> <out.html>
// Inputs (same folder): a-new-adventure-chronicle.md, chronicle.css, session-images.json
import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
const HERE = path.dirname(fileURLToPath(import.meta.url));
const DIR = process.argv[2] || HERE;
const OUT = process.argv[3] || path.join(DIR, 'a-new-adventure-chronicle.html');
...
<section class="session" id="s${s.num}" data-theme-art="${theme}" style="--gx:${gx}%;--gy:${gy}%">
  <header class="hero">
    ...
    <div class="hero-inner">
      <div class="kicker">A New Adventure · Chapter ${parseInt(s.num,10)}</div>
...
const md=fs.readFileSync(DIR+'/a-new-adventure-chronicle.md','utf8');
...
const html=`<title>A New Adventure Chronicle</title>
...
<div class="chronicle">
  <aside class="rail">
    <a class="rail-back" href="/campaigns/a-new-adventure" aria-label="Back to A New Adventure campaign">&larr; <span>A New Adventure</span></a>
    <div class="rail-head">
      <div class="rail-title">A New Adventure<br>Chronicle</div>
      <div class="rail-sub">A living account, as the party tells it</div>
    </div>
    <nav><ol class="rail-list">${rail}</ol></nav>
    <div class="rail-foot">${sessions.length} chapters &middot; A New Adventure</div>
  </aside>

  <main class="scroll">
${sessions.map(renderSession).join('\n')}
    <footer class="pagefoot">Suwanee Gamers &middot; A New Adventure Chronicle</footer>
  </main>
</div>
