// The Emberstran Chronicle builder — turns hoe-chronicle.md into a single self-contained HTML page.
// Usage:  node build-chronicle.mjs            (reads/writes this folder)
//         node build-chronicle.mjs <dir> <out.html>
// Inputs (same folder): hoe-chronicle.md, chronicle.css, session-images.json
import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
const HERE = path.dirname(fileURLToPath(import.meta.url));
const DIR = process.argv[2] || HERE;
const OUT = process.argv[3] || path.join(DIR, 'emberstran-chronicle.html');
...
function parseSession(lines){
  ...
  const num=head[1]; const title=head[2].replace(/\\!/g,'!').replace(/\\/g,'').trim();
  ...
  while(i<lines.length){
    ...
    if(/^IT IS INITIATIVE/i.test(line.trim())){blocks.push({t:'combat',v:line.trim()});i++;continue;}
...
const md=fs.readFileSync(DIR+'/hoe-chronicle.md','utf8');
...
const html=`<title>The Emberstran Chronicle</title>
...
<div class="chronicle">
  <aside class="rail">
    ...
    <div class="rail-foot">${sessions.length} chapters &middot; Heroes of Emberstran</div>
  </aside>

  <main class="scroll">
    ${sessions.map(renderSession).join('\n')}
    <footer class="pagefoot">Suwanee Gamers &middot; The Emberstran Chronicle</footer>
  </main>
</div>
